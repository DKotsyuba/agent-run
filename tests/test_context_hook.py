...
class ContextHookTests(unittest.TestCase):
    ...
    def test_unbound_session_returns_bounded_text_without_a_receipt(self) -> None:
        result = build_context(self.store, self.ref, config=self.config, now=1000.0)
        self.assertIsNone(result.orchestrator_session_id)
