...
class CapacityRankingTests(unittest.TestCase):
    ...
    def test_equal_current_capacity_is_ordered_by_forecast(self) -> None:
        """Burn history changes route order when current percentages are equal."""

        burning = _route(
            "runtime-a", "route-a", "pool-a",
            (_forecast("runtime-a", "lane-a", 80.0, burn=20.0, span=7_200.0),),
        )
        steady = _route(
            "runtime-b", "route-b", "pool-b",
            (_forecast("runtime-b", "lane-b", 80.0, burn=0.0, span=7_200.0),),
        )

        order = rank_capacity_routes(_snapshot(burning, steady), {}, now=_NOW)

        self.assertEqual([route.runtime for route in order.routes], ["runtime-b", "runtime-a"])
        self.assertEqual([route.windows[0].projected_percent for route in order.routes], [80.0, 60.0])
