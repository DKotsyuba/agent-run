...
class CiRetryTests(unittest.TestCase):
    ...
    def test_release_workflows_publish_verified_lock_before_smoke_and_checksums(self) -> None:
        ...
        for workflow in (ci, release):
            ...
            self.assertIn("--output-file dist/build-requirements.lock", workflow)
            self.assertLess(workflow.index("uv export --frozen"), workflow.index("requirements.lock"))
            wheel = workflow.index('python -m venv "$RUNNER_TEMP/wheel-smoke"')
            wheel_lock = workflow.index("-r dist/requirements.lock", wheel)
            wheel_artifact = workflow.index("-m pip install --no-deps dist/*.whl", wheel)
