...
class SnapshotServer:
    ...
    def _result(self, method: str, params: dict[str, object]) -> dict[str, object]:
        ...
        if method == "list_orchestrators":
            return {"items": [], "total": 0, "limit": params["limit"], "complete": True}
        if method == "list_agents":
            return {"items": [AGENT], "total": 1, "offset": params["offset"],
...
class MainTests(unittest.TestCase):
    ...
    def test_socket_option_builds_a_loader_that_reads_a_snapshot(self) -> None:
        """``--socket`` binds the real loader to that path and it yields a Snapshot."""
        with tempfile.TemporaryDirectory(dir="/tmp") as directory:
            ...
            self.assertEqual([card.agent_id for card in snapshot.agents["unbound"]], ["a1"])
