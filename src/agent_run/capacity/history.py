...
def load_series(
    ...
    for row in rows:
        key = CapacityKey(
            str(row["runtime"]), str(row["lane"]), str(row["window"]),
            cast(str | None, row["target"]), str(row["source"]),
        )
        grouped.setdefault(key, []).append(
            NormalizedSample(
                cast(float | None, row["remaining_percent"]),
                cast(float | None, row["reset_at"]),
                cast(float | None, row["observed_at"]),
                cast(float | None, row["valid_until"]),
            )
        )
    return tuple(
