...
For Claude and GLM, the index also hashes `.agent-run-plugin-launch.json`, which
records the ordered `--plugin-dir` paths and plugin-name roots used by the first
launch. A continuation reads these values only after index verification. Older
verified homes without this file reconstruct the paths from their stored launch
runtime and profile; unavailable legacy plugin sources fail the continuation
with an integrity error rather than silently dropping plugin flags.
