...
not gate or add an artifact to the 0.13.0 release.
...
cargo xtask release build-native --output "$release_root" --version 0.13.0
cargo xtask release verify --release "$release_root/releases/0.13.0"
cargo xtask archive --revision HEAD \
  --output "$release_root/agent-run-0.13.0-source.tar" --verify
```
...
3. Create and push an annotated tag from that accepted commit:
   ...
   ```bash
   git tag -a v0.13.0 -m "agent-run 0.13.0"
