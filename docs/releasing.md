...
not gate or add an artifact to the 0.12.1 release.
...
cargo xtask release build-native --output "$release_root" --version 0.12.1
cargo xtask release verify --release "$release_root/releases/0.12.1"
cargo xtask archive --revision HEAD \
  --output "$release_root/agent-run-0.12.1-source.tar" --verify
```
...
3. Create and push an annotated tag from that accepted commit:
   ...
   ```bash
   git tag -a v0.12.1 -m "agent-run 0.12.1"
