...
cargo xtask release verify --release "$release_root/releases/0.14.0"
cargo xtask archive --revision HEAD \
  --output "$release_root/agent-run-0.14.0-source.tar" --verify
```
...
3. Create and push an annotated tag from that accepted commit:
   ...
   ```bash
   git tag -a v0.14.0 -m "agent-run 0.14.0"
