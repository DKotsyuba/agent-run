...
/// Reads the latest persisted window per `(lane, source, window)` for one
/// runtime and global account, oldest-safe ordering irrelevant to callers.
///
...
fn latest_exhausted_windows(
    ...
    runtime: &str,
    account: &AccountId,
) -> Result<BTreeMap<(String, String, String), QuotaWindow>> {
    let mut stmt = tx.prepare(
        "SELECT lane,source,window,remaining_percent,reset_at,observed_at,valid_until FROM \
         (SELECT *,ROW_NUMBER() OVER(PARTITION BY lane,source,window ORDER BY observed_at DESC,id DESC) \
    ...
    for row in rows {
        let (lane, source, window, remaining, reset_at, observed_at, valid_until) = row?;
        if remaining != Some(0.0) {
            continue;
        }
        exhausted.insert(
            (lane.clone(), source.clone(), window.clone()),
            QuotaWindow {
                source,
                name: window,
...
pub fn retain_exhausted(
    exhausted: &BTreeMap<(String, String, String), QuotaWindow>,
    ...
    for model in &mut snapshot.models {
        for pool in &mut model.pools {
            ...
            // Positive fresh evidence releases the latch; an explicit unknown
            // for the same window is displaced by the retained fact instead of
            // duplicating it, and an expired reset releases it unconditionally.
            let fresh_known = |windows: &[QuotaWindow], source: &str, name: &str| {
                windows
                    .iter()
                    .any(|w| w.source == source && w.name == name && w.remaining_percent.is_some())
            };
            let mut carried: Vec<QuotaWindow> = Vec::new();
            for ((l, source, name), fact) in exhausted {
                if l != &lane
                    || fact.reset_at.is_some_and(|reset| reset <= at)
                    || fresh_known(&pool.windows, source, name)
                {
                    continue;
                }
                pool.windows
                    .retain(|w| !(w.source == *source && w.name == *name));
...
pub fn record_quota_snapshot(
    ...
    // Pool key -> explicit model membership, shared by every window of the pool.
    let mut membership: BTreeMap<&PhysicalQuotaKey, BTreeSet<&str>> = BTreeMap::new();
