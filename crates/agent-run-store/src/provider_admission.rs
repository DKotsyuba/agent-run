...
use crate::{admission, tx_event, Record, Store, ACTIVE_SQL};
...
impl Store {
    ...
    pub fn admit_provider(
        ...
        for candidate in &candidates.candidates {
            ...
            let key = (candidate.rank, load, candidate.account.as_str());
        ...
        let summary: String = request
            ...
            .collect();
        tx.execute(
            "INSERT INTO agents(id,request_id,orchestrator_session_id,runtime,model,profile,task,task_summary,workdir,request_json,status,created_at,timeout_seconds,config_revision,root_agent_id,sequence,identity_json,selection_intent,requested_account_id) \
             VALUES(?,?,?,?,?,?,?,?,?,?,'starting',?,?,'pending:provider-v2',?,1,?,?,?)",
            params![id.as_str(), request.request_id, session, request.provider.as_str(),
                ...
                at, effective.timeout_seconds.unwrap_or(480.0), id.as_str(),
