...
/// Mirrors `tests/test_native_settings.py::QwenNativeSettings::test_declared_settings_land_and_sandbox_stays_owned`.
#[test]
fn qwen_settings_land_while_sandbox_stays_owned() {
    let root = tempfile::tempdir().expect("temporary root");
    let settings = BTreeMap::from([(
        "advance".into(),
        toml::Value::Table(toml::map::Map::from_iter([(
    ...
    let (_, _, home, _) = materialize_runtime(root.path(), "qwen", settings);
    let document: serde_json::Value =
        serde_json::from_slice(&std::fs::read(home.join(".qwen/settings.json")).expect("settings"))
            .expect("JSON");
    assert_eq!(document["tools"]["sandbox"], true);
    assert_eq!(document["advance"]["thinking"], true);
}

/// Mirrors `tests/test_native_settings.py::QwenNativeSettings::test_security_and_capability_roots_rejected`.
#[test]
fn qwen_security_and_capability_roots_fail_closed() {
    let root = tempfile::tempdir().expect("temporary root");
    for key in [
        "tools",
        "security",
        ...
        let settings = BTreeMap::from([(key.into(), toml::Value::Integer(1))]);
        let runtime = runtime(root.path(), "qwen", settings);
        assert!(
            validate(
                &request("qwen", "fixture", root.path()),
                &runtime,
                &profile()
            )
            .is_err(),
