...
The supported 0.12.x artifact is the sealed macOS Apple-silicon release.
Linux builds are validation-only. Each release also includes a verified source
archive, `SHA256SUMS`, and provenance.
...
switches `current` atomically. It does not control launchd or perform the final
API/MCP smoke.

After an update, restart the launchd jobs and verify version, database
integrity, doctor, API `ping`/`tools`, MCP `initialize`/`tools/list`, and one
provider-free broker run. Preserve the deployment journal and backup after any
failure. Use the journal-aware recovery command; never manually point an older
binary at a newer schema.
