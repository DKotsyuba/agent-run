...
Domain / error / file primitives → configuration and profiles → state and adapters → service / supervisor → dispatcher → transports / CLI. Capacity and delivery depend on state and their bounded native transports, not on CLI argument parsing. The optional Desktop bridge owns only access to the signed host channel; notice construction and validation stay in Rust.
