...
`implemented` below means code exists, not that it has passed Rust compilation or a real-provider integration test. `test authored` means a Rust test is included, not executed. The actual checks and their results are recorded in `../validation/REPORT.md`.
...
12. This source archive does not include `Cargo.lock`: registry resolution was unavailable. The transitive dependency graph, declared toolchain floor, code formatting and all compiler diagnostics are unverified.
