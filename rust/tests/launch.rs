...
fn ready_child_is_a_session_leader_and_reaped_with_its_exact_status() {
    ...
    assert_eq!(unsafe { libc::getsid(pid) }, pid, "child must lead its session");
    // SAFETY: getpgid takes one PID and no pointers.
    assert_eq!(unsafe { libc::getpgid(pid) }, pid, "child must lead its group");
...
fn readiness_timeout_kills_the_verified_wrapper_and_grandchild_group() {
    ...
    assert!(started.elapsed() < Duration::from_secs(6), "cleanup must be bounded");
...
fn ready_eof_failure_token_and_bad_identity_are_refused_and_reaped() {
    ...
    for (script, expected) in [
        ("echo $$ >&4; exit 3", "supervisor exited before reporting ready"),
        (
...
fn a_missing_executable_is_refused_before_any_spawn() {
    ...
    let result = launch::launch_detached(
        Path::new("/nonexistent/agent-run"),
        &[],
        quick(),
        |_, _| {
            spawned = true;
    ...
    assert_eq!(failure.failure_kind, launch::FAILURE_KIND_EXECUTABLE_MISSING);
    assert_eq!(failure.stage.as_deref(), Some("preflight"));
    assert_eq!(failure.provisional_pid, None);
    assert!(failure.message.contains("reconnect/restart this MCP session"));
}
...
fn spawn_errors_propagate_and_the_fork_fallback_records_exec_failure() {
    ...
    let error = launch::spawn_session_leader(missing, &[], [&ready_w, &identity_w, &error_w])
        .unwrap_err();
    ...
    assert!(
        record.starts_with(r#"{"stage":"exec","type":"OSError""#) && record.contains(r#""errno":2}"#),
        "{record}"
    );
    let mut status = 0;
...
fn the_reaper_takes_only_its_own_registered_child() {
    ...
    let status = launch::spawn_reaper(child).unwrap().join().unwrap().unwrap();
    assert!(libc::WIFEXITED(status) && libc::WEXITSTATUS(status) == 7);
    let mut other = 0;
    // SAFETY: blocking waitpid on the exact unrelated child with a valid status pointer.
    assert_eq!(unsafe { libc::waitpid(unrelated, &mut other, 0) }, unrelated);
...
fn a_stale_leader_identity_is_never_signalled() {
    ...
    let refused = launch::terminate_group(&stale, Duration::from_millis(50), Duration::from_millis(50))
        .unwrap();
    assert!(refused.signals.is_empty(), "a reused identity must not be signalled");
    assert!(!refused.group_gone);
    assert!(child.try_wait().unwrap().is_none(), "the process must survive");
