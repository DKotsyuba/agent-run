...
use agent_run::{
    ...
    verify::{self, ANSWER_INCOMPLETE, ANSWER_PRESENT, ENGINE_VANISHED, GROUP_SURVIVED, NO_ANSWER},
};
use std::path::Path;
...
fn inspect_sentinel_makes_the_answer_complete_and_hashed() {
    ...
    assert_eq!(
        ...
        Some(agent_run::fs::sha256(body.as_bytes()).as_str())
    );
...
fn success_with_a_complete_answer_passes_through() {
    ...
    assert_eq!(answer.sha256, Some(agent_run::fs::sha256(body.as_bytes())));
    assert_eq!(answer.size_bytes, body.len() as u64);
}
