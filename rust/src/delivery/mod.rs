...
fn escaped(value: Option<&str>, missing: &str) -> String {
    ...
    for c in value.unwrap_or(missing).chars().take(128) {
...
fn guidance(kind: Option<&str>, status: Status) -> (&'static str, &'static str) {
    if status == Status::TimedOut {
        return (
            "Legacy runtime deadline was reached.",
            "Inspect the transcript before deciding whether to resume.",
        );
    }
    match kind {
        Some("provider_overloaded") => (
            "The provider is temporarily overloaded.",
            "Retry later or choose another compatible runtime/model.",
        ),
        Some("quota_exhausted" | "rate_limited") => (
            "The provider quota or rate limit blocked the run.",
            "Inspect limits and select a fresh compatible route.",
        ),
        Some("auth_error") => (
            "Runtime authentication was rejected.",
            "Check the configured account with agent-run doctor.",
        ),
        _ if status == Status::Lost => (
            "The recorded supervisor no longer owns a live process.",
            "Inspect answer and transcript; do not assume execution succeeded.",
        ),
        _ => (
            "The run did not produce a verified successful outcome.",
            "Inspect answer and transcript before taking any further action.",
        ),
    }
}
impl Notice {
    pub fn validate(&self) -> Result<()> {
        if !self.status.terminal()
            || !self.notification_id.starts_with("ntf_")
            || self.notification_id.len() <= 4
    ...
    pub fn render(&self) -> Result<String> {
        ...
        if matches!(
            self.status,
            Status::Failed | Status::Lost | Status::TimedOut
        ) {
            let (reason, advice) = guidance(self.failure_kind.as_deref(), self.status);
...
impl Evidence {
    fn new(classifier: &str, accepted: bool, ambiguous: bool) -> Self {
        Self {
...
fn claim(home: &Path) -> Result<Option<Claim>> {
    ...
    let Some((delivery_id, agent_id, sid, attempt)) = found else {
        return Ok(None);
    };
    let (transport, session) = tx.query_row(
        "SELECT transport,external_session_id FROM orchestrator_sessions WHERE id=?",
        [sid],
        |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)),
    )?;
    let (runtime, model, raw): (String, String, String) = tx.query_row(
        "SELECT runtime,model,request_json FROM agents WHERE id=?",
        [&agent_id],
    ...
    let effort = serde_json::from_str::<Value>(&raw)
        .ok()
        .and_then(|v| v.get("effort").and_then(Value::as_str).map(str::to_owned));
    let (status, kind): (String, Option<String>) = tx.query_row(
        "SELECT status,failure_kind FROM agents WHERE id=?",
        [&agent_id],
        |r| Ok((r.get(0)?, r.get(1)?)),
    )?;
    let ntf = if delivery_id.starts_with("ntf_") {
        delivery_id.clone()
    } else {
        format!("ntf_{}", &fs::sha256(delivery_id.as_bytes())[..32])
    };
    let bounded = |v: String| {
        if !v.trim().is_empty() && v.chars().count() <= 128 {
            Some(v)
        } else {
            None
        }
    ...
    let notice = Notice {
        ...
        model: bounded(model),
        effort: effort.and_then(bounded),
    ...
    tx.execute(
        "UPDATE deliveries SET state='sending',attempts=?,lease_owner=?,lease_until=? WHERE id=?",
        params![attempt, lease, time + 30., delivery_id],
    )?;
    tx.commit()?;
    Ok(Some(Claim {
        delivery_id,
...
fn complete(home: &Path, claim: &Claim, evidence: &Evidence) -> Result<()> {
    ...
    let state = if evidence.accepted {
        "delivered"
    } else if exhausted || evidence.classifier == "unsupported_transport" {
        "failed"
    } else {
        "retry_wait"
    };
    let backoff = (cfg.delivery.retry_base_seconds
        * 2f64.powi((claim.attempt.saturating_sub(1)).min(24) as i32))
    .min(cfg.delivery.retry_cap_seconds);
    ...
    tx.execute("UPDATE deliveries SET state=?,lease_owner=NULL,lease_until=NULL,next_attempt_at=?,last_error=?,ambiguous_result=? WHERE id=?",params![state,time+backoff,if evidence.accepted{None}else{Some(evidence.classifier.as_str())},evidence.ambiguous,claim.delivery_id])?;
    tx.commit()?;
    Ok(())
}
pub async fn dispatch_once(home: &Path) -> Result<usize> {
    let Some(claim) = claim(home)? else {
        return Ok(0);
    };
...
async fn claude_send(session: &str, notice: &Notice) -> Evidence {
    let execute = async {
        ...
        let host = std::env::var_os("HOME").ok_or_else(|| invalid("HOME missing"))?;
        let registry = std::path::PathBuf::from(host).join(".claude/sessions");
        let dir = fs::Dir::open(&registry)?;
        let mut entries = std::fs::read_dir(&registry)?
            .take(513)
            .collect::<std::io::Result<Vec<_>>>()?;
        if entries.len() > 512 {
            return Err(invalid("registry bound exceeded"));
        }
        ...
        for entry in &entries {
            ...
            let Ok(raw) = dir.read(Path::new(&name), 64 * 1024) else {
                continue;
            };
            ...
            if v["sessionId"].as_str() == Some(session) {
                if let (Some(pid), Some(path)) =
                    (v["pid"].as_i64(), v["messagingSocketPath"].as_str())
                {
                    if pid > 1 {
                        found = Some((pid, std::path::PathBuf::from(path)));
                        break;
                    }
                }
            }
        ...
        if !meta.file_type().is_socket() || meta.uid() != unsafe { libc::geteuid() } {
            return Err(invalid("untrusted session socket"));
        }
        let mut token = None;
        for entry in &entries {
            let name = entry.file_name();
            let label = name.to_string_lossy();
            if !label.starts_with(&format!("{pid}.")) || !label.ends_with(".key") {
                continue;
            }
            let file = dir.open_file(Path::new(&name))?;
            let meta = file.metadata()?;
            if meta.mode() & 0o077 != 0 {
                return Err(invalid("unsafe inbox key permissions"));
            }
            let raw = dir.read(Path::new(&name), 8192)?;
        ...
        let mut stream = tokio::time::timeout(Duration::from_secs(1), UnixStream::connect(path))
            .await
            .map_err(|_| invalid("session connect timeout"))??;
        let text = notice.render()?;
        if text.len() > 4096 {
            return Err(invalid("notice too large"));
        }
        let data = format!(
            "{}\n{}\n",
        ...
        match tokio::time::timeout(Duration::from_secs(5), stream.write_all(data.as_bytes())).await
        {
            Ok(Ok(())) => Ok::<Evidence, crate::Error>(Evidence::new("uds_written", true, false)),
            _ => Ok::<Evidence, crate::Error>(Evidence::new("uds_ambiguous", false, true)),
        }
    };
    match execute.await {
        Ok(evidence) => evidence,
        Err(_) => Evidence::new("uds_unavailable", false, false),
    }
}
