...
impl Store {
    ...
    fn connect(home: &Path, create: bool) -> Result<Self> {
        ...
        if version == 0 && create {
            ...
            if let Err(e) = conn.execute_batch(include_str!("schema.sql")) {
    ...
    pub fn finish(
        ...
        if let Some(proof) = proof {
            crate::verify::read(&self.home.join("agents").join(id.as_str()), proof, 0)?;
        }
        let tx = self
            .conn
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
    ...
    pub fn list(
        ...
        offset: usize,
        limit: usize,
        session: Option<&crate::domain::OrchestratorRef>,
        ...
        ) = d
        {
            ...
            (
                ...
                evidence
                    ...
                    .and_then(|v| crate::delivery::safe_evidence(&v))
