...
pub fn terminate_group(leader: &Identity, grace: Duration, kill: Duration) -> io::Result<Termination> {
    let pgid = leader.pid;
...
pub fn report_identity(identity_fd: RawFd, error_fd: RawFd) -> io::Result<()> {
    ...
    if session != pid {
        ...
        let record = serde_json::json!({"stage":"identity","type":"ValidationError","message":message});
...
fn cstring(bytes: &[u8]) -> io::Result<CString> {
    CString::new(bytes).map_err(|_| {
        io::Error::new(io::ErrorKind::InvalidInput, "launch argument contains NUL")
    })
}
...
fn posix_spawn_setsid(plan: &Prepared) -> io::Result<Option<i32>> {
    ...
    let spawned = (|| {
        ...
        for (source, target) in plan.sources.iter().zip([READY_FD, IDENTITY_FD, ERROR_FD]) {
            ...
            check(unsafe { libc::posix_spawn_file_actions_adddup2(actions, source.as_raw_fd(), target) })?;
        }
        for target in 0..3 {
        ...
        check(unsafe {
            libc::posix_spawn(&mut pid, plan.program.as_ptr(), actions, attr, argv.as_ptr(), envp.as_ptr())
        })?;
        Ok(Some(pid))
    })();
...
fn prove_identity(pid: i32, identity: &OwnedFd, error: &OwnedFd, deadline: Instant) -> Result<(), LaunchError> {
    ...
    let line = match read_line(identity.as_raw_fd(), deadline, 32) {
        ...
        Ok(Line::Eof) => return Err(LaunchError::Bootstrap(Box::new(diagnose(pid, error.as_raw_fd())))),
    ...
    if reported <= 1 {
        return Err(refused("detached supervisor reported an unsafe process identity"));
    }
    if reported != i64::from(pid) {
        return Err(refused("detached supervisor reported the wrong process identity"));
    }
    Ok(())
}
...
fn read_record(fd: RawFd) -> Option<serde_json::Map<String, Value>> {
    ...
    let first = buffer[..read as usize].split(|byte| *byte == b'\n').next()?;
...
fn describe_status(status: Option<i32>) -> String {
    match status {
        ...
        Some(status) if libc::WIFEXITED(status) => format!("exit code {}", libc::WEXITSTATUS(status)),
...
fn cleanup(pid: i32, leader: Option<&Identity>, timeouts: Timeouts) -> Result<(), String> {
    ...
    let reaped = wait_child(pid, timeouts.grace + timeouts.kill + Duration::from_millis(250)).is_some();
