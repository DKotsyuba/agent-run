    ...
    loop {
        ...
        if available.is_empty() {
            return if line.is_empty() {
                ...
                Err(Error::Runtime("truncated JSON frame".into()))
            };
        }
        let end = available.iter().position(|b| *b == b'\n');
        let n = end.map(|i| i + 1).unwrap_or(available.len());
        if line.len() + n > max {
            return Err(Error::Runtime("JSON frame exceeds maximum size".into()));
        }
...
pub async fn write<W: AsyncWrite + Unpin>(
    ...
    data.push(b'\n');
    if data.len() > max {
        return Err(Error::Runtime("JSON response exceeds maximum size".into()));
    }
