    ...
    let constraints = Constraint::ALL
        ...
        .map(|constraint| {
            let (enforcement, supported, reason) = match constraint {
                Constraint::WebToolsDisabled if !profile.network => (
                    Enforcement::ToolFilter,
                    true,
                    "native web tools are disabled; this is not network containment",
                ),
                Constraint::PluginImmutability if runtime.plugins.is_empty() => (
                ...
                _ => (
                    ...
                    false,
                    "no independently verified enforcement claim in this Rust port",
                ),
            };
            Evidence {
                constraint,
                enforcement,
                supported,
                required: profile.required_constraints.contains(&constraint),
...
impl EffectivePolicy {
    pub fn admit(&self) -> Result<()> {
        if self.constraints.iter().any(|e| e.required && !e.supported) {
            return Err(invalid("required policy constraints are not enforced"));
        }
        Ok(())
    }
}
