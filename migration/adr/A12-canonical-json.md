...
Add `rust/src/canonical.rs`: a byte-exact reimplementation of CPython's
...
`rust/src/fs.rs:236-257` already has a `canonical_json` helper (recursive
...
Its only call site is `rust/src/service.rs:109`, which fingerprints an
in-flight `StartRequest` into `replay_request_sha256` inside Rust's own
`LaunchIdentity` (`rust/src/service.rs:22-51`) — written and read back only
by Rust (`rust/src/state/mod.rs:261-269`), with a semantic-equality
...
- `adapters/materialize.rs:145-171` (`Publisher::finish`/`verify`): Rust's
  ...
  confirmed incompatible in `rust/docs/MIGRATION_STATUS.md`).
...
`LaunchIdentity::read` (`rust/src/service.rs:35-51`) explicitly refuses to
...
- Test vectors: `rust/tests/fixtures/canonical/vectors.json` — 64 primitive
...
- `rust/tests/canonical.rs`: `primitives_match_python_byte_for_byte` and
